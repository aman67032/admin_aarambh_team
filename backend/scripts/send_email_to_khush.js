...
async function main() {
  try {
    ...
    // 2. Fetch student
    const student = await Student.findOne({ sno: 429 });
    if (!student) {
      console.log("Student with S.No 429 not found.");
      return;
    }
    ...
    if (result.success) {
      ...
      print(`[FAILED] Welcome email failed: ${result.error}`);
    }
