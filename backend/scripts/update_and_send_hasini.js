...
async function main() {
  try {
    ...
    // 1. Update Prince Yadav's mobile number
    const newPrinceMobile = '`+91-8290439736';
    const prince = await Student.findOne({ name: /PRINCE YADAV/i });
    if (prince) {
      const oldMobile = prince.mobile;
      prince.mobile = newPrinceMobile;
      await prince.save();
      console.log(`[SUCCESS] Updated Prince Yadav's mobile: ${oldMobile} -> ${newPrinceMobile}`);
    } else {
      console.log('[ERROR] Prince Yadav not found in database.');
    }

    // 2. Update Ashwary Sharma's email
    const newAshwaryEmail = 'ramkumarsharmar02283@gmail.com';
    const ashwary = await Student.findOne({ name: /ASHWARY SHARMA/i });
    if (ashwary) {
      const oldEmail = ashwary.email;
      ashwary.email = newAshwaryEmail;
      await ashwary.save();
      console.log(`[SUCCESS] Updated Ashwary Sharma's email: ${oldEmail} -> ${newAshwaryEmail}`);

      // 3. Send welcome email to Ashwary Sharma
      console.log(`\nPreparing to send welcome email to Ashwary Sharma (${newAshwaryEmail})...`);
      ...
      if (result.success) {
        console.log(`[SUCCESS] Welcome email successfully sent to Ashwary Sharma!`);
      } else if (result.queued) {
        console.log(`[QUEUED] Welcome email queued for Ashwary Sharma (rate limit reached).`);
      } else {
