...
const TARGET_SNOS = [419, 420, 421, 422, 423, 424, 425, 426, 427];
...
async function main() {
  try {
    ...
    // 2. Fetch new students
    const students = await Student.find({ sno: { $in: TARGET_SNOS }, mailReceived: false });
    console.log(`Found ${students.length} students to send welcome emails.`);

    for (const student of students) {
      console.log(`\n--- Processing: ${student.name} (S.No: ${student.sno}) ---`);
      ...
      console.log(`Cohort: ${student.cohort} | Leader: ${clName} (${clEmail})`);
      ...
      if (result.success) {
        console.log(`[SUCCESS] Welcome email sent to ${student.name}!`);
        student.mailReceived = true;
        await student.save();
      } else if (result.queued) {
        console.log(`[QUEUED] Welcome email queued for ${student.name} (rate limit reached).`);
        student.mailReceived = true; // Mark as true since it is queued and will be sent automatically
        await student.save();
      } else {
        console.log(`[FAILED] Welcome email failed for ${student.name}: ${result.error}`);
      }
    }
